...
/**
 * Master generation pipeline (spec section 59). Runs as a long-lived
 * background worker (never inside a Vercel serverless function) so a
 * multi-minute multi-scene generation never hits a request timeout.
 */
async function processVideoProductionJob(job: Job<VideoProductionJobData>): Promise<VideoProductionJobResult> {
  const { organizationId, projectId, productInput, targetSceneNumbers } = job.data;
  ...
  try {
    ...
    await job.updateProgress({ step: "CREATIVE_STRATEGY", progress: 5 });
    const strategy = await orchestrator.executePipeline(productInput, async (step) => {
      await job.updateProgress({ step, progress: 10 });
    ...
    await job.updateProgress({ step: "GENERATING_SCENES", progress: 30 });
    ...
    // --- Voice synthesis ---
    await job.updateProgress({ step: "VOICE_SYNTHESIS", progress: 70 });
    ...
    // --- Captions ---
    await job.updateProgress({ step: "CAPTION_GENERATION", progress: 78 });
    ...
    await job.updateProgress({ step: "AUDIO_MIX", progress: 82 });
    ...
    // --- Final render ---
    await job.updateProgress({ step: "FINAL_RENDERING", progress: 90 });
