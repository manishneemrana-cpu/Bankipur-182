...
This describes the target architecture for the SitesNSign AI Voice Sales Agent Platform. No application code is written in Phase 0; this is a design document to be validated before Phase 1 build begins.
...
Conversation Orchestrator  (turn management, tool-calling, business logic, guardrails)
        │
        ├──► PostgreSQL + pgvector   (tenant data, call transcripts, CRM records, knowledge-base embeddings for RAG tool calls)
...
- **Conversation Orchestrator**: holds the conversation state machine, prompt/persona per tenant, tool-calling (e.g., "check property availability," "book a site visit," "check EMI eligibility" — could call other agents already in this repo's existing SitesNSign AI Executive skill roster, e.g. Home Loan/EMI Agent, Site Visit Coordinator, via internal API), and safety/compliance guardrails (e.g., refusing to place a call to a DND-registered number, enforcing consent windows).
- **PostgreSQL + pgvector**: system of record for tenants, agents, call logs, transcripts, CRM entities, and vector embeddings for retrieval-augmented responses (e.g., property listing lookups).
...
## Multi-tenancy / white-label / reseller considerations (for Phase 1 design, noted here for continuity)

- Every adapter (telephony/STT/TTS/LLM) selection, prompt/persona, and branding must be tenant-configurable, since resale/white-label is a first-class goal per the founder's spec.
